...
for filepath in allOMRs:
    ...
    inOMR = cv2.imread(filepath, cv2.IMREAD_UNCHANGED)

    options = TEMPLATES[squad].options
    # Align using refernce image?
    if 'ReferenceImage' in options:
        inOMR, h = alignImages(inOMR, options['ReferenceImage'])

    # Check if color channel specified in template
    if 'Channel' in options:    
        # use only one color channel
        inOMR = inOMR[:,:,{'b':0,'g':1,'r':2}[options['Channel'].lower()]]
    else:
        inOMR = cv2.cvtColor(inOMR, cv2.COLOR_BGR2GRAY)
